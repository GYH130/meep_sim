...
DEFAULT_SUBSTRATE_THICKNESS_UM = 25.0
DEFAULT_AIR_BUFFER_UM = 12.0
DEFAULT_PML_UM = 4.0

FLAT = "flat_Ti"
BARE = "scaled_bare_slanted_groove"
FILM_250 = "scaled_inner_wall_film_250nm"
FILM_500 = "scaled_inner_wall_film_500nm"
PROXY = "unpolarized_2D_proxy"
ALL_CASES = [FLAT, BARE, FILM_250, FILM_500]
DYNAMIC_FILM_PREFIX = "scaled_inner_wall_film_"
...
def _paths(output_tag: str | None = None) -> dict[str, Path]:
    ...
    if output_tag:
        ...
        return {
            ...
            "geom_bare": fig_dir / f"{tag}_geometry_scaled_bare_groove.png",
            "geom_250": fig_dir / f"{tag}_geometry_scaled_inner_wall_film_250nm.png",
            "geom_500": fig_dir / f"{tag}_geometry_scaled_inner_wall_film_500nm.png",
            ...
            "enhancement": fig_dir / f"{tag}_enhancement_over_scaled_bare.png",
    ...
    return {
        "spectra": project_path(
            ...
            "diagnostics_v2",
            "tables",
            "D11_scaled_deep_groove_inner_wall_film_spectra.csv",
        ),
        "spectra_checkpoint": project_path(
            "results",
            "diagnostics_v2",
            "tables",
            "D11_scaled_deep_groove_inner_wall_film_spectra_checkpoint.csv",
        ),
        "metrics": project_path(
            "results",
            "diagnostics_v2",
            "tables",
            "D11_scaled_deep_groove_inner_wall_film_metrics.csv",
        ),
        "resolution": project_path(
            "results",
            "diagnostics_v2",
            "tables",
            "D11_scaled_deep_groove_resolution_check.csv",
        ),
        "pml": project_path(
            "results",
            "diagnostics_v2",
            "tables",
            "D11_scaled_deep_groove_pml_check.csv",
        ),
        "report": project_path(
            "results",
            "diagnostics_v2",
            "reports",
            "D11_scaled_deep_groove_inner_wall_film_validation_report.md",
        ),
        "log": project_path(
            "logs",
            "diagnostics_v2",
            "D11_scaled_deep_groove_inner_wall_film_validation.log",
        ),
...
def _setup_worker_logger(path: Path) -> logging.Logger:
    ensure_dir(path.parent)
    logger = logging.getLogger(f"D11_scaled_deep_groove.worker.{os.getpid()}")
    logger.setLevel(logging.INFO)
    logger.propagate = False
    logger.handlers.clear()
    fmt = logging.Formatter(
...
def _is_inner_wall_film_case(case_name: str) -> bool:
    return case_name in {FILM_250, FILM_500} or _dynamic_case_thickness(case_name) is not None


def _case_thickness(case_name: str) -> float:
    if case_name == FILM_250:
        return 0.25
...
def _case_label(case_name: str) -> str:
    ...
    return {
        FLAT: "flat Ti",
        BARE: "scaled bare groove",
        FILM_250: "scaled 250 nm film",
        FILM_500: "scaled 500 nm film",
...
def _configure_args(args) -> None:
    ...
    if unknown:
        raise ValueError(
            ...
            f"{DYNAMIC_FILM_PREFIX}<thickness>{DYNAMIC_FILM_SUFFIX}, e.g. scaled_inner_wall_film_1um."
        )
    ...
    for thickness in film_check_thicknesses:
        if thickness == 0:
            rows.append({
                ...
                "status": "PASS",
                "details": "D11 uses build_slanted_groove_geometry for bare case and the geometry API degenerates film_thickness_um=0 to that same bare geometry.",
            })
            continue
...
def _raise_on_failed_geometry_checks(checks: pd.DataFrame) -> None:
    ...
    if not failed.empty:
        raise ValueError("D11 geometry checks failed:\n" + failed.to_string(index=False))
...
def _apply_refine_gate(args, paths: dict[str, Path], logger: logging.Logger) -> None:
    ...
    if not paths["metrics"].exists():
        raise RuntimeError(
            "Refine mode requires an existing D11 screen metrics CSV. "
            "Run --mode screen first and only refine if the 500 nm case reaches mean_A_strict >= 0.60."
        )
    ...
    screen_500 = screen_metrics[
        (screen_metrics["case_name"] == FILM_500)
        & (screen_metrics["polarization"].isin(["Ez", "Hz", PROXY]))
    ].copy()
    best_500 = float(screen_500["mean_A_8p1014_12p398_strict"].max()) if not screen_500.empty else np.nan
    if not np.isfinite(best_500) or best_500 < CONTINUE_THRESHOLD:
        raise RuntimeError(
            "Refine is blocked because screen did not show "
            f"{FILM_500} reaching mean_A_strict >= {CONTINUE_THRESHOLD:g}; "
            f"best available value is {best_500}."
        )
    if FILM_250 not in args.cases and FILM_250 in set(screen_metrics["case_name"]):
        screen_250 = screen_metrics[
            (screen_metrics["case_name"] == FILM_250)
            & (screen_metrics["polarization"].isin(["Ez", "Hz", PROXY]))
        ]
        best_250 = float(screen_250["mean_A_8p1014_12p398_strict"].max()) if not screen_250.empty else np.nan
        if np.isfinite(best_250) and best_250 > best_500:
            args.cases.append(FILM_250)
            logger.info(
                "Adding %s to refine because screen best %.6g exceeds 500 nm best %.6g",
                FILM_250,
                best_250,
                best_500,
            )
...
def _run_one(case_name: str, pol: str, wl: float, resolution: int, pml_um: float, args, logger) -> dict:
    ...
    result = run_periodic_2d_metal_single_wavelength(
        ...
        solver_version="scaled_deep_inner_wall_film_single_wavelength_v2",
...
def _add_enhancements(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in metrics.iterrows():
        ...
        for ref_name, out_name in [(FLAT, "flat_Ti"), (BARE, "scaled_bare")]:
...
def _plot_metric_bars(metrics: pd.DataFrame, paths: dict[str, Path]) -> None:
    ...
    enh = main[main["case_name"].isin([FILM_250, FILM_500])].copy()
    if "enhancement_over_scaled_bare_absolute" in enh.columns and not enh.empty:
        labels = [f"{_case_label(r.case_name)}\n{r.polarization}" for r in enh.itertuples()]
        x = np.arange(len(enh))
        ax.bar(x, enh["enhancement_over_scaled_bare_absolute"].astype(float), color="#59A14F")
...
def _run_best_field_snapshot(spectra: pd.DataFrame, metrics: pd.DataFrame, paths: dict[str, Path], args, logger) -> dict:
    ...
    if candidates.empty:
        return _write_placeholder_field_figures(
            paths,
            "Screen did not reach mean_A_strict >= 0.30 for the 500 nm inner-wall film case.",
            args,
        )
    ...
    film, _ = get_measured_lossy_wall_film_medium_single_wavelength(peak_wl, args.nk_csv)
    ti = get_ti_medium()
    air = mp.Medium(epsilon=1.0)
    ...
    logger.info(">>> D11 field snapshot case=%s pol=%s wl=%.6g", FILM_500, pol, peak_wl)
    sim.run(
    ...
    _plot_field_map(e2, f"D11 |E|^2, {pol}, {peak_wl:g} um", paths["best_e2"], extent, args, thickness)
    _plot_field_map(h2, f"D11 |H|^2, {pol}, {peak_wl:g} um", paths["best_h2"], extent, args, thickness)
    _plot_geometry(thickness, paths["best_overlay"], args)
    return {
        "field_snapshot_status": "COMPLETED",
        "field_snapshot_case": FILM_500,
...
def _md_table(metrics: pd.DataFrame) -> str:
    cols = [
        ...
        "peak_A_strict",
        "enhancement_over_scaled_bare_absolute",
...
def _best_500(metrics: pd.DataFrame) -> pd.Series | None:
    main = metrics[
        (metrics["case_name"] == FILM_500)
...
def _synergy_text(metrics: pd.DataFrame) -> str:
    ...
    for pol in ["Ez", "Hz", PROXY]:
        ...
        film = _metric_lookup(metrics, FILM_500, pol)
...
def _write_report(
    ...
    best_text = (
        "No strict quantitative 500 nm metric is available."
        if best is None
        else (
            f"Best 500 nm strict mean A = {best_mean:.6g} "
            f"({best['polarization']}, res={int(best['resolution'])}, "
    ...
    if pass80:
        ...
        decision = (
            "SCALED_INNER_WALL_FILM_ROUTE_FAIL_FOR_HIGH_EMISSIVITY or below the continuation "
            "threshold. This scaled groove + 500 nm inner-wall film structure should not be "
            "treated as the main high-emissivity route and should not automatically enter "
            "angle-resolved directionality optimization."
        )
    ...
    answers = f"""1. Scaled geometry built?
   Yes, if geometry checks are PASS: P={args.period_um:g} um, top/bottom width={args.top_width_um:g}/{args.bottom_width_um:g} um, depth={args.depth_um:g} um, tilt={args.tilt_angle_deg:g} deg, substrate={args.substrate_thickness_um:g} um.
...
3. 500 nm improvement over scaled bare groove:
   Ez: {_value_text(rows_500['Ez'], 'enhancement_over_scaled_bare_absolute')} absolute; Hz: {_value_text(rows_500['Hz'], 'enhancement_over_scaled_bare_absolute')} absolute; 2D proxy: {_value_text(rows_500[PROXY], 'enhancement_over_scaled_bare_absolute')} absolute.
...
6. Does it reach mean_A_strict >= 0.60?
   {'Yes' if pass60 else 'No'}. {best_text}

7. Does it reach mean_A_strict >= 0.80?
   {'Yes' if pass80 else 'No'}.
...
def _run_tasks(tasks: list[tuple[str, str, float, int, float]], args, paths: dict[str, Path], logger) -> list[dict]:
    ...
    logger.info(
        "Prepared %d D11 single-wavelength tasks; workers=%d; task_timeout_s=%s (not enforced)",
        len(tasks),
        workers,
        args.task_timeout_s,
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Optional dynamic inner-wall film thicknesses. Generates cases such as scaled_inner_wall_film_1um.",
    )
    ...
    parser.add_argument(
        "--output-tag",
        default=None,
        help="Optional filename prefix for all D11 outputs, used to run independent jobs without overwriting files.",
    )
    return parser
...
def main() -> None:
    ...
    try:
        ...
        _raise_on_failed_geometry_checks(geometry_checks)
        if is_master:
            _generate_geometry_plots(paths, args)
