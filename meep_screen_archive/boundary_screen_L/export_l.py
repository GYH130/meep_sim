"""Checkpoint L evidence without network access, Meep execution or legacy edits.
...
PREFIX = Path('meep_screen_archive/boundary_screen_L')
...
def main(final=False, label=None):
    ...
    with (exports / 'export.lock').open('a+') as lock:
        ...
        raw_name = 'L_raw_evidence_' + generation + '.tar.gz'
        compact_name = 'L_compact_' + generation + '.tar.gz'
        with tempfile.TemporaryDirectory(prefix='snapshot_', dir=exports) as temporary:
            ...
            atomic_json(temporary / 'L_SNAPSHOT.json', metadata)
            raw_temp = temporary / raw_name
            with tarfile.open(raw_temp, 'w:gz') as archive:
                for name, path in files.items():
                    archive.add(path, arcname='boundary_screen_L/' + name, recursive=False)
                archive.add(temporary / 'L_SNAPSHOT.json', arcname='boundary_screen_L/L_SNAPSHOT.json')
                if (temporary / 'budget_ledger_snapshot.json').exists():
                    archive.add(temporary / 'budget_ledger_snapshot.json', arcname='boundary_screen_L/budget_ledger_snapshot.json')
            ...
            for name, path in files.items():
                if ('cache' not in Path(name).parts and path.suffix in COMPACT_SUFFIXES
                    ...
                    add(path, name)
            add(temporary / 'L_SNAPSHOT.json', 'L_SNAPSHOT.json')
            if (temporary / 'budget_ledger_snapshot.json').exists():
                add(temporary / 'budget_ledger_snapshot.json', 'budget_ledger_snapshot.json')
            atomic_json(dest / 'L_FILES.json', inventory)
