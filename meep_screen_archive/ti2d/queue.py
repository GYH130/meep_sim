...
class Queue:
    ...
    def _snapshot_locked(self, state, active=None, message=None):
        ...
        summary = {"run_id":self.manifest["run_id"], "state":state,
            ...
            "remaining_solver_seconds":max(0., self.budget["total_active_seconds"]
                                           - self.ledger.total_active_seconds()),
    ...
    def execute(self, task, attempt, admission=None):
        ...
        remaining = self.budget["total_active_seconds"] - self.ledger.total_active_seconds()
        ...
        with FileLock(task_dir / "task.lock"), (task_dir / "stdout.log").open("wb") as log:
            try:
                ...
                if exit_code and result.get("status") in ("QUALIFIED","MISSING_OUTPUT"):
                    status = "CRASHED"
                if status:
                ...
                self.ledger.close_interval(record,solver_end_epoch or time.time(),final_status)
        ...
        with FileLock(self.run_dir / "queue.lock"), FileLock(self.ledger.path.with_suffix(".lock")):
            ...
            for task in self.manifest["tasks"]:
                ...
                if self.cancelled:
                    final = "INTERRUPTED"
                    break
                if self.ledger.total_active_seconds() >= self.budget["total_active_seconds"]-30:
